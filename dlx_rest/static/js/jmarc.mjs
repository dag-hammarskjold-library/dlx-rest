...
export class Jmarc {
	...
	// Take two arrays, one of headers, the other of a data row, and make a Jmarc object out of them
	static async fromCsv(collection, headers, dataRow) {
		if (!["bibs", "auths"].includes(collection)) {
			throw new Error("First argument must be \"bibs\" or \"auths\"")
		}
		...
		for (let jdx in headers) {
			...
			} else if (fieldName.length === 3 && fieldName('00')) {
				tag = fieldName
				code = null
			}
