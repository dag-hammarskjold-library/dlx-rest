<template>
  <div class="subfield-row" @keydown.capture="onSubfieldShortcut">
    ...
    <span
      v-else
      ref="valueEl"
      :class="['subfield-value', classes.subfieldValue, { 'subfield-value__invalid': isSubfieldValueInvalid }]"
      :title="isSubfieldValueInvalid ? 'Invalid subfield value for this code' : null"
      :contenteditable="!readonly"
      @keydown="keyDown"
      @input="setValue"
...
export default {
  ...
  methods: {
    ...
    getValidationDocument() {
      const collectionMap = { bibs: 'bibs', speeches: 'speeches', votes: 'votes', auths: 'auths' }
    ...
    getAuthorityDisplayText(authRecord) {
      ...
      for (const tag of tagKeys) {
        ...
        const text = subfields.map(sf => (sf && sf.value ? String(sf.value).trim() : '')).filter(Boolean).join(' ')
        if (text) return text
      }
      ...
      if (Array.isArray(authRecord.fields)) {
        ...
        if (headingField && Array.isArray(headingField.subfields)) {
          const text = headingField.subfields.map(sf => (sf && sf.value ? String(sf.value).trim() : '')).filter(Boolean).join(' ')
          if (text) return text
        }
      }
      return ''
    ...
    getAuthorityHeadingSubfields(authRecord) {
      ...
      const normalizeSubfields = rawSubfields => {
        ...
        return rawSubfields
          .map(sf => ({ code: sf && typeof sf.code === 'string' ? sf.code : '', value: sf && sf.value != null ? String(sf.value) : '' }))
    ...
    getAuthorityControlledCodes() {
      ...
      if (tagMap && typeof tagMap === 'object') return Object.keys(tagMap)
    ...
    handleClickOutside(event) {
      ...
      if (menuContainer && !menuContainer.contains(event.target)) this.showMenu = false
    ...
    finalizeValue(event) {
      if (this.readonly) return
      const trimmed = event.target.innerText.trim()
      this.subfield.value = trimmed
    ...
    onCodeFocus() { this.openCodeMenu() },
    onValueFocus() { this.openValueMenu() },
    ...
    onCodeTriggerKeyDown(event) {
      ...
      event.preventDefault()
      if (!this.showCodeMenu) this.openCodeMenu()
    ...
    onValueTriggerKeyDown(event) {
      ...
      if (event.key !== 'ArrowDown' && event.key !== 'ArrowUp') return
      event.preventDefault()
      if (!this.showValueMenu) this.openValueMenu()
      const delta = event.key === 'ArrowDown' ? 1 : -1
    ...
    onCodeOptionKeyDown(event, index) {
      ...
      if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
        event.preventDefault()
        this.focusCodeOption(index + (event.key === 'ArrowDown' ? 1 : -1))
      }
    },
    onValueOptionKeyDown(event, index) {
      ...
      if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
        event.preventDefault()
        this.focusValueOption(index + (event.key === 'ArrowDown' ? 1 : -1))
      }
    },
    onSubfieldShortcut(event) {
      ...
      if (isAddShortcut) { this.addSubfield(); return }
      this.deleteSubfield()
    },
    async searchAuthorities(query) {
      ...
      try {
        ...
        if (!params.has(this.subfield.code)) {
          ...
          if (activeValue) params.append(this.subfield.code, activeValue)
        }
        ...
        if (response.ok) {
          ...
          const records = Array.isArray(data) ? data : Array.isArray(data && data.data) ? data.data : []
          ...
          if (this.authSearchResults.length === 0) {
            this.authSearchResults.push({ _id: 'not-found', value: 'Authority not found', notFound: true })
    ...
    computeAuthDropdownDirection() {
      this.$nextTick(() => {
        ...
        this.authDropdownUp = spaceBelow < 320
      })
    },
    selectAuthority(authority) {
      ...
      if (authoritySubfields.length > 0 && this.field && typeof this.field.getSubfield === 'function' && typeof this.field.createSubfield === 'function') {
        ...
        for (const authSubfield of authoritySubfields) {
          if (hasControlledCodeList && !controlledCodeSet.has(authSubfield.code)) continue
          incomingCodes.add(authSubfield.code)

          let targetSubfield = this.field.getSubfield(authSubfield.code)
          if (!targetSubfield) targetSubfield = this.field.createSubfield(authSubfield.code)

          targetSubfield.value = authSubfield.value
          if (authorityId) targetSubfield.xref = authorityId
        }
        ...
        if (incomingCodes.size > 0) {
          ...
          for (const code of incomingCodes) {
            ...
            for (let i = 1; i < sameCode.length; i++) this.field.deleteSubfield(sameCode[i])
          }
        }
      } else {
        this.subfield.value = authority.heading || authority.value || ''
        if (authorityId) this.subfield.xref = authorityId
      }
    ...
    handleAuthValueFocusOut(event) {
      ...
      if (event.currentTarget && !event.currentTarget.contains(nextTarget)) {
        ...
        if (!isInDropdown) this.showAuthSearch = false
      }
    },
    ...
    onAuthValueKeyDown(event) {
      ...
      if ((event.key === 'ArrowDown' || event.key === 'ArrowUp') && this.authSearchResults.length > 0) {
        ...
        this.moveAuthSelection(event.key === 'ArrowDown' ? 1 : -1)
        return
      }
    ...
    onAuthOptionKeyDown(event, index) {
      ...
      if (event.key === 'Enter') {
        ...
        if (authority && !authority.notFound) this.selectAuthority(authority)
        return
      }
      if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
        event.preventDefault()
