<template>
  ...
  >
    ...
    <div v-if="isLockedByOther" class="record-lock-summary">
      <div class="record-lock-summary-title">Locked for editing</div>
...
import { FieldClipboardService } from '../services/FieldClipboardService.js'
import { WorkformService } from '../services/WorkformService.js'
...
export default {
  ...
  emits: ['focus-record', 'clone-record', 'open-related-record', 'batch-actions', 'save-record', 'saveRecord', 'unlock-record', 'delete-record', 'close-record', 'authLookupChoices', 'pasteFields'],
  data() {
    return {
      hasChanges: false,
      isSaving: false,
      ...
      controls: [
        { id: 'undo', label: 'Undo', icon: 'bi-arrow-counterclockwise', permission: 'updateRecord', action: 'undoChange' },
        { id: 'redo', label: 'Redo', icon: 'bi-arrow-clockwise', permission: 'updateRecord', action: 'redoChange' },
        { id: 'save', label: 'Save', icon: 'bi-floppy', permission: 'updateRecord', action: 'saveRecord' },
        { id: 'clone', label: 'Clone', icon: 'bi-files', permission: 'createRecord', action: 'cloneRecord' },
        { id: 'paste', label: 'Paste Fields', icon: 'bi-clipboard', permission: 'updateRecord', action: 'pasteFields' },
        { id: 'history', label: 'History', icon: 'bi-clock-history', permission: 'updateRecord', action: 'openHistoryModal' },
        { id: 'save-workform', label: 'Save as Workform', icon: 'bi-file-earmark-plus', permission: 'createWorkform', action: 'saveAsWorkform' },
        { id: 'delete', label: 'Delete Record', icon: 'bi-trash', permission: 'deleteRecord', action: 'deleteRecord' },
        { id: 'batch', label: 'Batch Actions', icon: 'bi-list-check', permission: 'updateRecord', action: 'batchActions' }
      ]
    }
  },
  computed: {
    ...
    validationErrors() {
      if (!this.canSeeValidationState) return []
      ...
      dataFields.forEach((field, index) => {
        ...
        requiredSubfields.forEach(requiredCode => {
          ...
          if (matchingSubfields.length === 0) {
            errors.push({ type: 'subfield-required-missing', tag: field.tag, index, code: requiredCode })
            return
          }

          const hasNonBlankValue = matchingSubfields.some(subfield => String(subfield.value || '').trim().length > 0)
          if (!hasNonBlankValue) {
            errors.push({ type: 'subfield-required-blank', tag: field.tag, index, code: requiredCode })
          }
        })
        ...
        field.subfields.forEach(subfield => {
          ...
          if (hasEmptyValue) {
            errors.push({ type: 'subfield-empty-value', tag: field.tag, code: subfield.code })
            return
          }
